...
path = '/Users/erikdewater/Downloads/'
setwd(path)
...
parallel::mclapply(1:5,function(i){
 SG_miss = SG_miss[, .(gest_age_weeks, SG, days,mothers_age, mother_bmi, phthalates)]
 SG_nomiss = SG_nomiss[, .(gest_age_weeks, SG, days,mothers_age, mother_bmi, phthalates)]
  train_XY <- SG_nomiss
  test_XY <-  SG_nomiss
  # this step takes a while.
  reg_auto_dart <- run.autoxgboost.dart(traindt = train_XY, target = 'SG')
  # predict to test dataset
  xgb_pred <- predict(reg_auto_dart, newdata=test_XY)
  SG_miss$pred<<- xgb_pred$data$response
  rmse_cv[i] <<- sqrt(mean((xgb_pred$data$truth - xgb_pred$data$response)^2))  
...
SG_imputed_median<-setDT(SG_imputed_median)

observed<- SG_nomiss[,.(SG)]
predicted<- SG_imputed_median[,.(SG_imputed_median)]


#create indicator variables for each predicted and observed SG #dataset
observed[!is.na(SG), SG_cat:= 1]
predicted[!is.na(SG_imputed_median), SG_cat:= 2]
setnames(predicted, "SG_imputed_median", "SG")


#combine observed and predicted values so that they both
#are in the same variable
SG_imp <- rbind(observed, predicted)

#give 'factor' class to indicator variable
SG_imp$SG_cat <-as.factor(SG_imp$SG_cat)
SG_imp[SG_cat== 1, SG_group:= "Observed Specific Gravity"]
SG_imp[SG_cat== 2, SG_group:= "Imputed Specific Gravity"]
SG_imp$SG_group <- as.factor(SG_imp$SG_group)

   
#density plot for observed and predicted SG
ggplot(SG_imp, aes(x = SG, color = SG_group)) + geom_density(alpha = 0.5) +
geom_hline(yintercept=0, colour="white", size=1) +
  ylab("Density")+
  xlab("Specific Gravity") +
  labs(color = "Specific Gravity Type") +
  theme(legend.title = element_text(size = 10, face = "bold")) + ggtitle("Density Plot of Observed and Imputed Specific Gravity") +
