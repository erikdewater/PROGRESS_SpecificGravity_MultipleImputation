...
```
##Use autoxgboost to predict missing values (change trainign set to subjects with measured SG, and testing set to those with missing values)
```{r}
run.autoxgboost.dart <- function(traindt, target, ...){
  require(autoxgboost)
  par_dart <- makeParamSet(
...
SG_data<-setDT(SG_data)
parallel::mclapply(1:5,function(i){
  tmp = SG_data[, .(gest_age_weeks, SG, days,mothers_age, mother_bmi, phthalates)]
  train_XY <- tmp[SG]
  test_XY <-  tmp[ Folds[[i]] ]
  # this step takes a while.
  reg_auto_dart <- run.autoxgboost.dart(traindt = train_XY, target = 'SG')
